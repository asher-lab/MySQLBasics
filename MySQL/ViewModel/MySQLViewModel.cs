...
namespace MySQL
{
    ...
    public class MySQLViewModel : BaseViewModel
    {
        ...
        /// <summary>
        /// The Salary of the student
        /// </summary>
        public string Salary { get; set; }

        /// <summary>
        /// The JoinDate of the student
        /// </summary>
        ...
        private Status SetStudentForm (string id, string firstName, string lastName, string age, string salary, string joindate, string phone  )
        {
        ...
        private (Status, Student) GetStudentFromId (int id)
        {
            ...
            for (int i = 0; i < Class.Count; i++)
            {
                if ((Class[i]?.ID ?? -1) == id)
                {
                    ...
                    student.Salary = Class[i]?.Salary ?? string.Empty;
                    student.JoinDate = Class[i]?.JoinDate ?? string.Empty;
                    student.Phone = Class[i]?.Phone ?? string.Empty;
                }
            }
        ...
        private (Status, Student) GetStudentFromIndex (int index)
        {
            ...
            student.Salary = Class[index]?.Salary ?? string.Empty;
            student.JoinDate = Class[index]?.JoinDate ?? string.Empty;
            student.Phone = Class[index]?.Phone ?? string.Empty;
                    ...
                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(query, connection))
                    {
                        ...
                        foreach (DataRow row in table.Rows)
                        {
                            ...
                            tempClasse.Add(new Student()
                            {
                                ...
                                Salary = row.ItemArray[4].ParseString(),
                                JoinDate = row.ItemArray[5].ParseString(),
                                Phone = row.ItemArray[6].ParseString(),
                            });
                        }
                    }
                }
                        ...
                        using (MySqlDataReader mysqlReader = command.ExecuteReader())
                        {
                            ...
                            {                              
                                ...
                                tempClasse.Add(new Student()
                                {
                                    ...
                                    Salary = mysqlReader.GetString(4),
                                    JoinDate = mysqlReader.GetString(5),
                                    Phone = mysqlReader.GetString(6),
        ...
        private Status InsertData()
        {
            ...
            string firstName = FirstName?.Trim(),  age = Age?.Trim(),  salary = Salary?.Trim(), joinDate = JoinDate?.Trim(), phone = Phone?.Trim();

            // Check if inputs are empty
            if (string.IsNullOrWhiteSpace(firstName) == true ||  string.IsNullOrWhiteSpace(age) == true)
                goto End;
            ...
            if (Regex.IsMatch (firstName, RegexDict[RegexType.Alpha]) == false ||
                ...
                Regex.IsMatch(Salary, RegexDict[RegexType.Alpha]) == false ||
                Regex.IsMatch(JoinDate, RegexDict[RegexType.Alpha]) == false ||
                Regex.IsMatch(Phone, RegexDict[RegexType.Alpha]) == false)

                goto End;

            // Format the insert request
            string query = "INSERT INTO test.classe(firstName, lastname, age, salary, joindate, phone ) VALUES('" + firstName + "', '"+firstName+"',  " + age + ", '" + salary + "', '" + joinDate + "', '" + phone + "'  )";
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        using (MySqlDataReader mysqlReader = command.ExecuteReader())
                        {
                            ...
                            {                                
                                ...
                                student.Salary = mysqlReader.GetString(4);
                                student.JoinDate = mysqlReader.GetString(5);
                                student.Phone = mysqlReader.GetString(6);
        ...
        private (Status, string) UpdateData ()
        {
            ...
            string id = ID?.Trim(), firstName = FirstName?.Trim(),  age = Age?.Trim(), salary = Salary?.Trim(), joindate = JoinDate?.Trim(), phone = Phone?.Trim() ;
            ...
            if (string.IsNullOrWhiteSpace(id) == true || string.IsNullOrWhiteSpace(firstName) == true ||
             string.IsNullOrWhiteSpace(age) == true)
            ...
            string query = "UPDATE test.classe SET firstName='" + firstName + "',  age =" + age + " ,  Salary='" + Salary + "', JoinDate='" + JoinDate + "', Phone='" + Phone + "'  WHERE id=" + id;
